	...
	public static class ReactiveAssert
	{
		...
		public static void AreElementsEqual<T>(IEnumerable<T> expected, IEnumerable<T> actual)
		{
			...
			if (!expected.SequenceEqual(actual))
				Assert.IsTrue(false, Message(actual, expected));
		}
		...
		public static void Throws<TException>(Action action) where TException : Exception
		{
			...
			{
				Assert.IsTrue(false, string.Format(CultureInfo.CurrentCulture, "Expected {0} threw {1}.\r\n\r\nStack trace:\r\n{2}", typeof(TException).Name, ex.GetType().Name, ex.StackTrace));
			}

			if (failed)
				Assert.IsTrue(false, string.Format(CultureInfo.CurrentCulture, "Expected {0}.", typeof(TException).Name));
		}
